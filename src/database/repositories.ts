...
export class Repositories {
  ...
  latestTopPools(limit = 5): Array<Record<string, unknown>> {
    const db = openDb();
    const rows = db.prepare("SELECT * FROM pool_scans ORDER BY id DESC LIMIT ?").all(limit) as Array<Record<string, unknown>>;
    db.close();
    return rows;
  }
  ...
  latestTradeLogs(limit = 10): Array<Record<string, unknown>> {
    const db = openDb();
    const rows = db.prepare("SELECT * FROM trade_logs ORDER BY closed_at DESC, id DESC LIMIT ?").all(limit) as Array<Record<string, unknown>>;
    db.close();
    return rows;
  }
